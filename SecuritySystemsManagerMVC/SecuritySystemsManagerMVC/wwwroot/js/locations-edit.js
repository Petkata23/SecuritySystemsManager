document.addEventListener("DOMContentLoaded", function () {
    ...
    var mapElement = document.getElementById('editLocationMap');
    if (!mapElement) {
        console.error("❌ Error: Map container with id='editLocationMap' not found.");
        return;
    }
    ...
    // Default to center of Bulgaria if no valid coordinates
    if (isNaN(lat) || isNaN(lng) || lat === 0 || lng === 0) {
        lat = 42.6977;
        lng = 23.3219;
        console.warn("⚠️ Invalid coordinates, using default center of Bulgaria.");
    }

    // Initialize map with colorful theme
    var map = L.map('editLocationMap', {
        zoomControl: false,
